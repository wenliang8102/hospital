...
async function loadDepartments() {
  loading.value = true
  try {
    const response = await http.get<ApiResponse<PageData<Department>>>('/master-data/departments', {
      params: {
...
async function saveDepartment() {
  ...
  try {
    if (form.id) {
      await http.put<ApiResponse<Department>>(`/master-data/departments/${form.id}`, payload)
      ElMessage.success('科室已更新')
    } else {
      await http.post<ApiResponse<Department>>('/master-data/departments', payload)
...
async function changeActive(row: Department) {
  ...
  await http.patch<ApiResponse<Department>>(`/master-data/departments/${row.id}/active`, { active: nextActive })
