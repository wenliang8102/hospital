...
class MasterDataLookupApiTest {
    ...
    void listsActiveEmployeesByDepartmentAndKeyword() throws Exception {
        ...
        mockMvc.perform(get("/api/master-data/employees")
                ...
                .andExpect(jsonPath("$.data[0].registrationLevelName").value("普通号"));
    }

    @Test
    void listsDiseasesByKeyword() throws Exception {
        ...
        mockMvc.perform(get("/api/master-data/diseases")
                ...
                .andExpect(jsonPath("$.data[0].name").value("普通感冒"))
                .andExpect(jsonPath("$.data[0].icd").value("J00"));
    }

    @Test
    void listsDrugsByNameCodeOrMnemonic() throws Exception {
        jdbcTemplate.update("""
        ...
        mockMvc.perform(get("/api/master-data/drugs")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].code").value("DRUG001"))
                .andExpect(jsonPath("$.data[0].name").value("阿莫西林胶囊"))
                .andExpect(jsonPath("$.data[0].price").value(12.50));
    }

    @Test
    void listsMedicalTechnologies() throws Exception {
        ...
        mockMvc.perform(get("/api/master-data/medical-technologies")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].code").value("MT001"))
                .andExpect(jsonPath("$.data[0].name").value("血常规"))
                .andExpect(jsonPath("$.data[0].departmentId").value(inspectionId));
    }
