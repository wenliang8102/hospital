...
- 每笔交易通过 `payment_transaction_item` 记录具体收费项和交易金额，支持可审计的部分支付与部分退款。
- 退款收费项必须属于指定原支付流水，且仍处于 `PAID`；已被医技接收、执行或已发药的项目不能直接退费。
- 医技申请、处方和收费项在同一事务中创建，项目价格以服务端基础数据为准。
- 作废仅允许 `CREATED` 业务单据及其 `UNPAID` 收费项一起转为 `CANCELLED` 和 `VOID`。
...
- 收费和退款以交易流水号保证幂等；重放时必须同时匹配交易类型、收费项集合和原支付流水。
